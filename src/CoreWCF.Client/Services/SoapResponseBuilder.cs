...
public static class SoapResponseBuilder
{
    ...
    private static TResponse DeserializeCatTypesResponse<TResponse>(XmlNode node, XmlNamespaceManager ns)
    {
        ...
        if (firstCatType?.NamespaceURI == "http://schemas.datacontract.org/2004/07/CoreWCF.Contracts")
        {
            ...
            var serializer =
                new DataContractSerializer(typeof(CatType[]), "CatTypes", "http://tempuri.org/");
            catTypes.AddRange((CatType[])serializer.ReadObject(xmlReader));
        }
        else
        {
            ...
            foreach (XmlNode catTypeNode in catTypesNode.ChildNodes)
            {
                ...
                var likesChildren =
                    bool.Parse(catTypeNode.SelectSingleNode("tem:LikesChildren", ns)?.InnerText ?? "false");

                catTypes.Add(new CatType
                {
                    RaceName = raceName,
